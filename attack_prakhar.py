...
def loadWAV(filename):
    ...
    feat = np.stack([audio], axis=0).astype(np.float) # n*2 <- left-ear, right-ear
    return feat
...
def Attack(speaker_model, enroll_speaker_feat, train_speaker, speaker, target, args):
    ...
    target_feat = torch.FloatTensor(enroll_speaker_feat[target])
    wav_input = torch.FloatTensor(wav_input_np)
    
    if torch.cuda.is_available():
        target_feat = target_feat.cuda()
        wav_input = wav_input.cuda()
    
    # Need to maximize the speaker/person detected
    ...
    for i in range(max_steps):
        ...
        for b in range(batch_num):
            ...
            # optimized tensors
            adv_noise = torch.FloatTensor(adv_noise_np)
            if torch.cuda.is_available():
            ...
            if not args.rir:
                ...
                rir_tensor = torch.FloatTensor(loadWAV(os.path.join(args.attack_rir_path, random.choice(rir_wavs_train))))
                if torch.cuda.is_available():
                    rir_tensor = rir_tensor.cuda()
            ...
            # print(wav_feat.shape, target_feat.shape)
            # print(target_feat)
            
            CROSS_ENTROPY = True
            if CROSS_ENTROPY and target_feat.shape[0] == 1:
                target_feat = torch.cat((target_feat, target_feat, target_feat, target_feat, target_feat), dim=0)
            
            # print(wav_feat.shape, target_feat.shape)
            # print(target_feat)

            # dist_batch = F.mse_loss(wav_feat, target_feat, requires_grad=True)
            # dist_batch = F.cosine_similarity(wav_feat, target_feat)
            dist_batch = F.pairwise_distance(wav_feat, target_feat)
            # dist_batch = F.cross_entropy(wav_feat, target_feat)

            print(dist_batch.shape)
            
            # loss1
            loss1 = torch.sum(torch.clamp(args.thresh - dist_batch + args.margine, min=0.))
            # loss1 = torch.norm(args.thresh - dist_batch + args.margine)
        ...
        noise_list_1[str(i)] = str(adv_noise_np)
    ...
    # Hyperparameters/Change model slightly
    ...
    for i in range(max_steps):
        ...
        for b in range(batch_num):
            ...
            # loss1
            loss1 = torch.sum(torch.clamp(args.thresh - dist_batch + args.margine, min=0.))
            # loss1 = torch.nn(args.thresh - dist_batch + args.margine)

            # loss2
            spec_stft = torch.stft(adv_noise/2**15, n_fft=320, hop_length=160, win_length=320, window=torch.hamming_window(320))

            if torch.cuda.is_available():
                spec_stft = spec_stft.cuda()

            loss2 = torch.mean(torch.sqrt(spec_stft[:,:,0]*spec_stft[:,:,0] + spec_stft[:,:,1]*spec_stft[:,:,1]))
        ...
        noise_list_2[str(i)] = str(adv_noise_np)
    ...
    return result1, step1, loss, result, step2, loss1_value, loss2_value, noise_list_1, noise_list_2
...
def Test(speaker_model, enroll_speaker_feat, test_speaker, speaker, target, adv_noise, args):
    ...
    for wav in wavs:
        ...
        wav_data_tensor = torch.FloatTensor(wav_data)
        if torch.cuda.is_available():
            wav_data_tensor = wav_data_tensor.cuda()
...
if torch.cuda.is_available():
    ...
    speaker_model = speaker_model(**vars(args))
...
with torch.no_grad():
    for k,v in enroll_speaker.items():
        ...
        for i in range(len(v)):
            ...
            wav_data = torch.FloatTensor(loadWAV(wav))
...
f_spectro = open(os.path.join(args.out_path, 'spectrogram.txt'), 'w')

#intra-gender attack
if args.intra_gender:
    print("Intra-gender attack ...")
    for i in range(len(original_speaker)//2):
        if i<args.num_start or i>=args.num_end: continue
        print('Num: %02d' % i)
        speaker = original_speaker[i]
        target = target_speaker[i]
        noise1, step1, loss, noise, step2, loss1_value, loss2_value, noise_list1, noise_list2 = Attack(speaker_model, enroll_speaker_feat, train_speaker, speaker, target, args)
        with torch.no_grad():
        ...
        f.write(out)
        f.flush()   
        # f_spectro.write(noise_list1)     # write spectrogram
        # f_spectro.write(noise_list2)
        # f.flush()
        
        print(noise_list1,len(noise_list1['0']))
        print(noise_list2,len(noise_list2['0']))
        
        with open("spectrogram.json","w") as outfile:
            json.dump(noise_list1, outfile)
            json.dump(noise_list2, outfile)
